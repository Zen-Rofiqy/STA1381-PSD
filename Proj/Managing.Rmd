...
3.  **Memberi Bobot pada setiap Benchmark\
    **`Chat GPT` memberi saran untuk nilai bobot masing-masing :\
    ![](images/Screenshot 2023-09-20 091849.png)\
    Dengan cara perkalian matriks antara data benchmark $(\mathbf{B})$ dengan matriks diagonal bobot $(\mathbf{D})$:
...
Agak disayangkan, kami blm berhasil membuat fungsi `join_and_score` bisa digunakan untuk data `gpu` juga. Tapi tidak apa, kami hanya perlu mengganti cpu ke gpu
...
find_similar <- function(target, data) {
  ...
  #adis untuk menghitung jarak (distance) antara target_gpu dan setiap model GPU
  ...
  best_match_model <- gpu_data$Model[best_match_index]
  return(best_match_model)
}

# Fungsi untuk mengMergekan data jika ada yang sama
join_and_score2 <- function(data, gpu) {
  #Buat Kolom baru, set sebagai NA
  data[, BestMatch := NA_character_]
  ...
  for (i in 1:nrow(data)) {
    ...
    #Jika tidak ada yang sama, maka cek kesamaan
      ...
      matching_rows <- cpu[stringdist::stringdist(tolower(Model), 
                                                tolower(current_cpu), 
                                                method = "jw") >= 0.9]
      matching_rows <- cpu[find_similar(current_gpu, gpu),]
...
# MengMergekan data "cpu" ke "data" dengan kondisi yang dioptimalkan
setDT(data)  # Mengubah data.frame menjadi data.table
setDT(gpu)   # Mengubah data.frame cpu menjadi data.table

#Panggil fungsi
coba <- join_and_score2(data[1:1000,], gpu)
```
...
```{r}
current_gpu <- "AMD Radeon RX Vega 6 (R2000/3000, 15W)"
current_gpu_no_parentheses <- gsub("\\([^)]+\\)", "", current_gpu)

coba <- gpu[stringdist::stringdist(
            #Cari Model tanpa tanda kurung yang mirip sekitar 90% 
            #dengan GPU data tanpa tanda kurung
                        tolower(gsub("\\([^)]+\\)", "", Model)), 
                        tolower(current_gpu_no_parentheses), 
                        method = "jw") >= 0.90, ]
install_load('fuzzywuzzyR')
# Fungsi untuk mencari model GPU yang paling mirip
find_similar <- function(target, data) {
...
current_gpu <- "AMD Radeon RX Vega 6 (R2000/3000, 15W)"
coba <- find_similar(current_gpu, gpu)
...
![](images/Screenshot 2023-09-20 153804.png)
