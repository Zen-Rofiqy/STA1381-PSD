...
```{r}
ols.bs <- as.data.frame( olsrr::ols_step_best_subset(reg.klas)  )
```
...
```{r}
reg.klas2 <- lm(mpg ~ cyl + hp + wt + am, data=data)
summary(reg.klas2)
```
...
```{r, warning=FALSE, message = FALSE}
install_load('randtests','lmtest')
# Melakukan uji asumsi
t_test <- t.test(reg.klas2$residuals, mu = 0, conf.level = 0.95)
runs_test <- runs.test(reg.klas2$residuals)
bp_test <- ncvTest(reg.klas2)
shapiro_test <- shapiro.test(reg.klas2$residuals)
...
```{r}
weights <- 1 / lm(abs(reg.klas2$residuals) ~ reg.klas2$fitted.values)$fitted.values^2

reg.klas2_wls <- lm(mpg ~ cyl + hp + wt + am, data=data, weights = weights)
summary(reg.klas2_wls)
```

```{r}
# Melakukan uji asumsi
t_test <- t.test(reg.klas2_wls$residuals, mu = 0, conf.level = 0.95)
runs_test <- runs.test(reg.klas2_wls$residuals)
bp_test <- ncvTest(reg.klas2_wls)
shapiro_test <- shapiro.test(reg.klas2_wls$residuals)
